...
def get_sim(pre, gold):
    d_model = pre.size(2)
    gold_ = gold.view(-1, d_model)
    pre_ = pre.contiguous().view(-1, d_model)

    return pre_, gold_


def get_diff(pre, gold):
    gold = gold.contiguous().view(-1)
...
def get_one_hot(gold):
    b, n, l = gold.size()
    gold_one_hot = torch.zeros([b, n, l], dtype=torch.long).to(gold.device)
    # gold_one_hot.scatter_(2, gold, 1).scatter_(2, torch.zeros(b, n, 1, dtype=torch.long).to(gold.device), 0)
    gold_one_hot.scatter_(2, gold, 1)
    gold_one_hot = gold_one_hot[:, :, :-1]
    gold_one_hot = gold_one_hot.float()
    return gold_one_hot
...
def pretrain_epoch(model, training_data, optimizer, device, opt):
    ...
    for batch in tqdm(training_data, mininterval=2, desc='  - (Training)   ', leave=False):
        ...
        # gold_one_hot = get_one_hot(batch_dif_sk_label)
        ...
        # ====task-difficult====
        pre_dif_q, gold_dif_q = get_diff(seq_logit_diff_q, batch_diff_label)
