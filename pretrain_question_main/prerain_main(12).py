...
def main():
    ...
    parser.add_argument('-q_sum', type=int, default=50983)
    parser.add_argument('-s_sum', type=int, default=198)
    parser.add_argument('-s_size', type=int, default=198)
    parser.add_argument('-log', default=None)
    parser.add_argument('-no_cuda', action='store_true')
    parser.add_argument('-sq_len', type=int, default=30)
    ...
    start = time.time()
    # =============data====================
    train_data, skill_data, diff_skill, dif = getData('2012', opt.sq_len)
    ...
    # =============pretain===============================
    print('pretrain............')
    pretrain_bert = BERTModel(q_size=opt.q_sum, s_size=opt.s_sum, s_label_size=opt.s_size, d_k=opt.d_k, d_v=opt.d_v, d_model=opt.d_model, d_inner=opt.d_inner_hid, n_layers=opt.n_layers, n_head=opt.n_head, dropout=opt.dropout).to(device)
    optimizer = ScheduledOptim(optim.Adam(pretrain_bert.parameters(), betas=(0.9, 0.98), eps=1e-09), opt.d_model,
                               opt.n_warmup_steps)
    pretrain(pretrain_bert, pretrain_data, optimizer, device, opt, '2012', writer)
