...
def main():
    ...
    print('*' * 30, '训练模型加载中......', '*' * 30)
    # ==========load pretrain==========================
    pretrain = BERTModel(q_size=opt.out_dim, s_size=opt.n_skill, s_label_size=opt.skill_size, d_k=16, d_v=16, d_model=128, d_inner=512,
                         n_layers=1, n_head=8, dropout=0.2).to(device)
    checkpoint = torch.load('../pretrain_chkpt/2012-30.chkpt')
